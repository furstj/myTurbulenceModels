...
if "kkLOmega" in model_name:
    ...
    u  = np.sqrt(velocity[:, 4]**2 + velocity[:, 5]**2)
else:
    velocity = np.loadtxt("postProcessing/line_z5/5000/line_k_omega_U.xy")
    tx = velocity[:, 0]
    k  = velocity[:, 1]
    u  = np.sqrt(velocity[:, 3]**2 + velocity[:, 4]**2)
...
plt.legend()
plt.tight_layout()
plt.savefig(f'cf_plot_{case_name}_{model_name}.png')
...
plt.legend()
plt.tight_layout()
plt.savefig(f'tu_plot_{case_name}_{model_name}.png')

# Create a freestream velocity 
plt.clf()
plt.plot(Re_x, u_interp, label='Freestream velocity $Tu$', color='magenta')
plt.scatter(exp_data[:, 1], exp_data[:, 2], label='Experimental Data', color='red', marker='o', s=50)
plt.xlabel('Reynolds Number $Re_x$')
plt.ylabel('Freestream velocity $U_0$ (m/s)')
plt.title(f'Freestream Velocity vs Reynolds Number\nCase: {case_name}, Model: {model_name}')
plt.ylim(0,15)
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.savefig(f'u_plot_{case_name}_{model_name}.png')
